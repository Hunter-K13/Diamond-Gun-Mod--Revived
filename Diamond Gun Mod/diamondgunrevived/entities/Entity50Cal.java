...
public class Entity50Cal extends Entity implements EntityShot {
	...
	public Entity50Cal(World world, EntityPlayer entityplayer) {
		super(world);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public Entity50Cal(World world, double d1, double d2, double d3) {
		super(world);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public Entity50Cal(World world, EntityLivingBase entityliving, float f0) {
		super(world);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		...
		setArrowHeading(this.motionX, this.motionY, this.motionZ, f0 * 1.5F, 1.0F);
	}
	...
	public void d(double d0, double d1, double d2) {
		...
		if ((this.prevRotationPitch == 0.0F) && (this.prevRotationYaw == 0.0F)) {
			float f0 = MathHelper.sqrt_double(d0 * d0 + d2 * d2);
			this.prevRotationYaw = (this.rotationYaw = (float)(Math.atan2(d0, d2) * 180.0D / 3.141592741012573D));
			this.prevRotationPitch = (this.rotationPitch = (float)(Math.atan2(d1, f0) * 180.0D / 3.141592741012573D));
	...
	public void setArrowHeading(double d0, double d1, double d2, float f0, float f1) {
		float f2 = MathHelper.sqrt_double(d0 * d0 + d1 * d1 + d2 * d2);
		d0 /= f2;
		d1 /= f2;
		d2 /= f2;
		d0 += this.rand.nextGaussian() * 0.007499999832361937D * f1;
